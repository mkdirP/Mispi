...
function redraw() {
    ...
    ctx.fillStyle = blue;
    ctx.fillRect(width / 2, height / 2, 2 / 6 * width, - 2 / 6 * height);
    ctx.beginPath();
...
// 这段代码用于将 HTML 表格中的数据提取为一个数组，该数组中的每个元素都是一个 Map 对象，表示一行数据，其中的键值对表示单元格的内容和对应的表头。
function getPoints() {
    const points = [];
